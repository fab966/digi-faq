...
use Joomla\CMS\Factory; // Per Joomla 4/5

class FaqHelper
{
    public static function getFaqItems(\Joomla\Registry\Registry $params)
    {
        $items = [];
    ...
    public static function addFaqSchema($renderMarkup, array $faqItems)
    {
        if (empty($faqItems)) {
            return;
        }
        ...
        if (!empty($schema['mainEntity']) && ($renderMarkup)) {
